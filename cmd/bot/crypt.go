...
func initCryptKey(cryptKey string) {
	if cryptKey == "" {
		log.Fatalln("Error crypt key not specified")
	}

	var err error
	key, err = hex.DecodeString(cryptKey)
	if err != nil {
		panic(err.Error())
	}
}
...
func decrypt(encryptedString string) string {
	bytes, _ := hex.DecodeString(encryptedString)
